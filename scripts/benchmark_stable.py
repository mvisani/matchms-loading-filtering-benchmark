"""Benchmark spectra loading and per-filter processing with matchms 0.33.1."""
...
def main(mgf: str, output: str, repeats: int, install_spec: str, limit):
    ...
    for _ in range(repeats):
        ...
        raw = load_from_mgf(mgf)
        spectra = []
        count = 0
        for spectrum in tqdm(raw, desc="Loading spectra", leave=False):
        ...
        n_loaded.append(len(spectra))

        working = [s.clone() for s in spectra]
        for name, fn in zip(FILTER_NAMES, filter_functions):
            ts0 = time.perf_counter()
            working = [fn(s) for s in working]
            ts1 = time.perf_counter()
            filter_stage_times[name].append(ts1 - ts0)

        fresh = [s.clone() for s in spectra]
        tc0 = time.perf_counter()
        processed = [default_filters(s) for s in fresh]
